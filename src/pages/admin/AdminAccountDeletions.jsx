  ...
  return (
    <AdminLayout>
      <div className="p-5 lg:p-8 max-w-3xl">
        <div className="mb-6">
          <h1 className="font-syne font-bold text-ink text-2xl">Account Deletions</h1>
          <p className="text-ink/40 font-dm text-sm mt-1">Review and process account-deletion requests</p>
        </div>
          ...
          {TABS.map((t) => (
            <button
              ...
              className={`px-4 py-2 rounded-xl font-dm text-sm whitespace-nowrap border transition-colors ${
                tab === t.value ? "bg-iris/15 text-iris border-iris/25" : "text-ink/50 border-line hover:text-ink"
              }`}
            >
              {t.label}
            </button>
        ...
        {loading ? (
          <div className="card-flat p-8 text-center text-ink/35 font-dm text-sm">Loading requests...</div>
        ) : requests.length === 0 ? (
          <div className="card-flat p-10 text-center">
            <FiUserX className="text-ink/20 mx-auto mb-3" size={28} />
            ...
            {requests.map((r) => (
              <div key={r.id} className="card-flat p-5 flex items-center justify-between gap-4">
                <div className="min-w-0">
                  ...
                  {r.reason && <p className="text-ink/40 font-dm text-xs mt-1 italic">"{r.reason}"</p>}
                </div>
                {r.status === "pending" && (
                  <div className="flex gap-2 shrink-0">
                    ...
                    <button
                      ...
                      disabled={busyId === r.id}
                      className="flex items-center gap-1.5 text-ink/50 hover:text-ink border border-line font-dm text-xs px-3 py-2 rounded-lg disabled:opacity-60"
                    >
                      <FiX size={13} /> Reject
                    </button>
                  </div>
