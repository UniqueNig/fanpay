  ...
  return (
    <AdminLayout>
      <div className="p-5 lg:p-8 max-w-5xl">
        <div className="mb-7">
          <h1 className="font-syne font-bold text-ink text-2xl">Transactions</h1>
          <p className="text-ink/40 font-dm text-sm mt-1">Every deposit, transfer, and payment across the platform</p>
        </div>
            ...
            {TYPE_FILTERS.map((f) => (
              <button
                ...
                className={`px-4 py-2 rounded-xl font-dm text-xs font-medium border transition-colors ${
                  type === f.value
                    ? "bg-iris/15 border-iris/25 text-iris"
        ...
        <div className="card-flat overflow-hidden">
          {loading ? (
            <div className="py-16 text-center text-ink/35 font-dm text-sm">Loading transactions...</div>
            ...
            <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
              <div className="w-16 h-16 rounded-2xl bg-surface border border-line flex items-center justify-center mb-4">
                <FiList size={22} className="text-ink/30" />
              </div>
              <p className="text-ink font-syne font-semibold text-base mb-1">No transactions found</p>
              <p className="text-ink/40 font-dm text-sm">Try a different search or filter</p>
            </div>
          ) : (
                ...
                <div className="flex items-center gap-4 shrink-0 ml-3">
                  {tx.uid && (
                    <Link
                      ...
                      className="hidden sm:block text-ink/40 hover:text-iris font-dm text-xs truncate max-w-[140px]"
