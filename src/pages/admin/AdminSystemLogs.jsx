...
const LEVEL_STYLE = {
  ...
  info: { color: "text-ink/50", icon: FiInfo },
};
  ...
  return (
    <AdminLayout>
      <div className="p-5 lg:p-8 max-w-3xl">
        <div className="mb-6">
          <h1 className="font-syne font-bold text-ink text-2xl">Cron Jobs & Logs</h1>
          <p className="text-ink/40 font-dm text-sm mt-1">Webhook errors and VTU reconciliation cron activity</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="card-flat p-10 text-center">
            <FiTerminal className="text-ink/20 mx-auto mb-3" size={28} />
            <p className="text-ink/40 font-dm text-sm">No log entries yet.</p>
          </div>
        ) : (
          <div className="card-flat overflow-hidden">
            {logs.map((log, i) => {
              ...
              return (
                <div key={log.id} className={`flex items-start gap-3 p-4 ${i < logs.length - 1 ? "border-b border-line" : ""}`}>
                  <Icon size={14} className={`mt-0.5 shrink-0 ${style.color}`} />
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <p className="text-ink font-dm text-sm">{log.source}</p>
                      <span className="text-ink/25 font-dm text-[11px]">{formatDate(log.timestamp)} · {formatTime(log.timestamp)}</span>
                    </div>
