...
const Savings = () => {
  ...
  const planColors = {
    flexi: { accent: "#00d4aa", bg: "bg-iris/10", border: "border-iris/20", text: "text-iris" },
  ...
  return (
    <DashboardLayout>
      <div className="p-5 lg:p-8 max-w-3xl">
        <div className="mb-7">
          <h1 className="font-syne font-bold text-ink text-2xl">Savings</h1>
          <p className="text-ink/40 font-dm text-sm mt-1">Grow your money with competitive interest rates</p>
        </div>

        {/* Savings overview */}
        <div className="card-flat p-6 mb-7 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <p className="text-ink/50 font-dm text-xs mb-1">Total in Savings</p>
            <p className="font-syne font-bold text-3xl text-gold">{formatNaira(savingsBalance)}</p>
          ...
          <div className="flex flex-col items-start sm:items-end gap-1">
            <div className="bg-gold/10 border border-gold/20 rounded-xl px-4 py-2">
              <p className="text-ink/40 font-dm text-[10px]">Wallet Balance</p>
              <p className="text-ink font-syne font-semibold text-sm">{formatNaira(balance)}</p>
            </div>
          </div>
        </div>
        ...
        {success ? (
          <div className="card-flat p-10 flex flex-col items-center text-center gap-5">
            <div className="w-16 h-16 rounded-full bg-iris/15 border border-iris/20 flex items-center justify-center">
              <FiCheckCircle size={30} className="text-iris" />
            </div>
            <div>
              <h2 className="font-syne font-bold text-ink text-xl mb-2">Savings Added! 🎉</h2>
              <p className="text-ink/50 font-dm text-sm">
                {formatNaira(parseFloat(amount))} added to your <strong className="text-ink">{selectedPlan?.name}</strong> plan.
                Earning {selectedPlan?.rate} per annum.
              </p>
            </div>
            <button onClick={() => { setSuccess(false); setSelectedPlan(null); setAmount(""); }} className="btn-iris">
              Save More
            </button>
          </div>
        ) : !selectedPlan ? (
          <div>
            <h2 className="font-syne font-semibold text-ink text-base mb-4">Choose a Savings Plan</h2>
              ...
              {SAVINGS_PLANS.map((plan) => {
                ...
                return (
                  <div
                    ...
                    className={`card-flat p-6 cursor-pointer hover:scale-[1.02] transition-all duration-200 border ${col.border} hover:${col.bg}`}
                  >
                    ...
                    <p className="text-ink/50 font-dm text-xs leading-relaxed mb-3">{plan.description}</p>
                    <div className="flex flex-col gap-1.5 text-xs font-dm text-ink/40">
          ...
          <div className="card-flat p-6">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="font-syne font-semibold text-ink text-base">{selectedPlan.name}</h2>
                <button onClick={() => setSelectedPlan(null)} className="text-iris font-dm text-xs hover:underline">
                  ← Change plan
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-6">
              {[
                ...
                <div key={i} className="bg-surface border border-line rounded-xl p-3">
                  <p className="text-ink/40 font-dm text-[10px] mb-1">{d.label}</p>
                  <p className="text-ink font-syne font-semibold text-sm">{d.val}</p>
                </div>
              ))}
            </div>
            ...
            <form onSubmit={handleSave} className="flex flex-col gap-4">
              <div>
                <label className="text-ink/60 font-dm text-xs mb-1.5 block">
                  Amount to Save (₦) — minimum {formatNaira(selectedPlan.min)}
                </label>
                <div className="relative">
                  <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-ink/40 font-syne font-bold">₦</span>
                  <input
                    type="number"
                    value={amount}
                    onChange={(e) => setAmount(e.target.value)}
                    className="input-field-light pl-8"
              ...
              {amount && parseFloat(amount) >= selectedPlan.min && (
                <div className="bg-iris/8 border border-iris/15 rounded-xl px-4 py-3 text-iris font-dm text-xs">
              ...
              <button
                ...
                className="btn-iris flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
